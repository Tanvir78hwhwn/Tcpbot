...
📱 Player will see:
"HN BLAZE wants to join your team"
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]HN BLAZE [00FF00]___X³____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @HNBLAZE\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
def talk_with_ai(question):
    url = f"https://gpt4.apisimpacientes.workers.dev/chat?prompt="
    res = requests.get(url)
    if res.status_code == 200:
        data = res.json()
...
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]@HNBLAZE
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="HNBLAZE", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="HNBLAZE", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde HN GAMING ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="HNBLAZE", title_id=title_id)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        if str(invite_uid) in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 14010319252
                                message_text = f" Can't accept Your request Talk to HN GAMING"
...
"""
                            ...
                            if str(response.Data.uid) != "4446036055":  # Replace with your actual UID
...
👥 Whitelisted UIDs:"""
                            ...
                            for uid in WHITELISTED_UIDS:
                                if uid != "4446036055":  # Skip owner since already shown
                                    whitelist_msg += f"\n• {uid}"
                                    count += 1
                                    if count >= 20:
...
[b][i][FFC0CB]youtube: HN GAMING 99[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @HNBLAZE[/b]

[b][c][FFA500]telegram contact: @HNBLAZE[A52A2A]
...
"""
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # If test works, send help
                            help_text = f"""[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[00FF00]ＷＥＬＣＯＭＥ [FFD700]{{player_name}}[00FF00] ＴＯ
[FF0000] _ _ _ X ³ _ _ _ _   [00FF00]ＢＯＴ
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━"""
    
                            await safe_send_message(response.Data.chat_type, help_text, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)    
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 HNBLAZE BOT - INITIALIZING")
    print("=" * 50)
